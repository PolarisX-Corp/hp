"use client";

import { useEffect, useRef, useState } from "react";
import type { TocItem } from "@/lib/blog/types";
import { Icon } from "./Icon";

/** 上部の読書プログレスバー */
export function ReadingProgress() {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const onScroll = () => {
      const h = document.documentElement;
      const ratio = h.scrollTop / (h.scrollHeight - h.clientHeight || 1);
      if (ref.current) {
        ref.current.style.width = `${Math.min(100, Math.max(0, ratio * 100))}%`;
      }
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    onScroll();
    return () => window.removeEventListener("scroll", onScroll);
  }, []);
  return <div className="bl-progress" ref={ref} />;
}

/** 目次 + スクロールスパイ */
export function TocNav({ toc }: { toc: TocItem[] }) {
  const [activeId, setActiveId] = useState<string>(toc[0]?.id ?? "");
  useEffect(() => {
    const onScroll = () => {
      let current = toc[0]?.id ?? "";
      for (const item of toc) {
        const el = document.getElementById(item.id);
        if (el && el.getBoundingClientRect().top <= 120) current = item.id;
      }
      setActiveId(current);
    };
    window.addEventListener("scroll", onScroll, { passive: true });
    onScroll();
    return () => window.removeEventListener("scroll", onScroll);
  }, [toc]);

  if (toc.length === 0) return null;
  return (
    <ul className="bl-toc">
      {toc.map((item) => (
        <li key={item.id}>
          <a href={`#${item.id}`} className={item.id === activeId ? "is-active" : ""}>
            {item.text}
          </a>
        </li>
      ))}
    </ul>
  );
}

const BRAND_PATHS: Record<string, string> = {
  x: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z",
  fb: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z",
  li: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z",
};

function BrandGlyph({ kind }: { kind: keyof typeof BRAND_PATHS }) {
  return (
    <svg viewBox="0 0 24 24" width={16} height={16} fill="currentColor" aria-hidden>
      <path d={BRAND_PATHS[kind]} />
    </svg>
  );
}

/** シェアボタン（X / Facebook / LinkedIn / リンクコピー） */
export function ShareButtons({ title }: { title: string }) {
  const [copied, setCopied] = useState(false);

  const open = (target: string) =>
    window.open(target, "_blank", "noopener,width=600,height=520");

  const share = (kind: string) => {
    const url = encodeURIComponent(window.location.href);
    const text = encodeURIComponent(title);
    if (kind === "x") open(`https://twitter.com/intent/tweet?url=${url}&text=${text}`);
    else if (kind === "fb") open(`https://www.facebook.com/sharer/sharer.php?u=${url}`);
    else if (kind === "li") open(`https://www.linkedin.com/sharing/share-offsite/?url=${url}`);
    else if (kind === "copy") {
      navigator.clipboard?.writeText(window.location.href);
      setCopied(true);
      window.setTimeout(() => setCopied(false), 1600);
    }
  };

  return (
    <div className="bl-share">
      <button className="bl-share__btn" aria-label="Xでシェア" onClick={() => share("x")}>
        <BrandGlyph kind="x" />
      </button>
      <button className="bl-share__btn" aria-label="Facebookでシェア" onClick={() => share("fb")}>
        <BrandGlyph kind="fb" />
      </button>
      <button className="bl-share__btn" aria-label="LinkedInでシェア" onClick={() => share("li")}>
        <BrandGlyph kind="li" />
      </button>
      <button
        className="bl-share__btn"
        aria-label={copied ? "コピーしました" : "リンクをコピー"}
        onClick={() => share("copy")}
      >
        <Icon name="link" size={16} />
      </button>
    </div>
  );
}
