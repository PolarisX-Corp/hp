# ブログ記事の投稿ガイド

PolarisX ブログ（`/blogs`）の記事は、**この `public/blog/` 配下に Markdown を追加し、GitHub で PR を作成する**ことで投稿します。CMS は使いません。マージされると自動的にサイトに公開されます。

## 1. 記事フォルダを作る

1 記事 = 1 フォルダです。`_template/` をコピーして、フォルダ名を記事の **slug**（URL に使う英小文字・ハイフン）にリネームします。

```
public/blog/
  your-article-slug/      ← このフォルダ名がURL: /blogs/your-article-slug
    index.md              ← 記事本体（必須）
    assets/               ← 画像置き場
      cover.png
      figure-1.png
```

> フォルダ名（slug）は半角英小文字・数字・ハイフンのみ。`_` 始まりは雛形扱いで公開されません。

## 2. frontmatter を記入する

`index.md` の先頭（`---` で囲まれた部分）に記事情報を書きます。

| キー | 必須 | 説明 |
| --- | --- | --- |
| `title` | ✅ | 記事タイトル |
| `description` | ✅ | 概要（一覧の抜粋・meta description。120字程度） |
| `category` | ✅ | 下記のカテゴリ key から1つ |
| `publishedAt` | ✅ | 公開日 `YYYY-MM-DD` |
| `author` | | 著者名（省略時は「PolarisX 編集部」） |
| `updatedAt` | | 更新日 `YYYY-MM-DD` |
| `cover` | | カバー画像（`assets/xxx.png`）。省略時はカテゴリ別の自動生成アート |
| `tags` | | タグの配列 |
| `featured` | | 一覧トップの「注目の記事」に出すなら `true` |

### カテゴリ key 一覧

`lib/blog/categories.ts` で定義された固定セットです。ここにある key を `category` に指定します。

| key | 表示名 |
| --- | --- |
| `ai-agent` | AIエージェント・AI社員 |
| `knowledge-base` | 社内ナレッジ・RAG |
| `ai-helpdesk` | 問い合わせ自動化 |
| `ai-adoption` | 法人AI導入 |
| `ai-workflow` | 業務効率化・AI活用術 |
| `ai-tools` | AIツールガイド |
| `case-study` | 導入事例・実践レポート |

`data-driven-management` / `bi` / `kpi` / `repro-score` / `data-integration` / `dx-org` / `product` は旧キーです。既存の公開記事の互換用に残しているだけなので、**新規記事では使わないでください**。

新しいカテゴリを増やしたい場合は `lib/blog/categories.ts` に追加してください（アイコン・カラーもそこで定義）。

## 3. 本文を書く

frontmatter の下に Markdown で本文を書きます。

- 使える記法: 見出し / 段落 / **太字** / _斜体_ / マーカー `==` / 箇条書き / 番号付きリスト / 引用 `>` / 表 / コード / リンク / 画像。
- **生の HTML は使えません**（安全のため無効化しています）。書いても表示されません。
- 読了時間は本文の文字数から自動計算されます。

> **記法の見本ページ**
> `npm run dev` して <http://localhost:3000/blogs/style-guide> を開くと、下記の記法をすべて並べた見本を実際のスタイルで確認できます（開発時のみ。本番では 404）。

### 記事内CTA（自動挿入）

相談への導線は記事テンプレートが自動で挿入します。執筆者がMarkdownへCTA用のHTMLやボタンを追加する必要はありません。

| 項目 | ルール |
| --- | --- |
| 主目的 | 記事の内容を自社へ応用するための**無料相談** |
| 文言 | カテゴリごとの課題に合わせて自動で出し分け |
| 遷移先 | トップページの問い合わせフォーム `/#contact` |
| 挿入数 | 1記事につき1つ |
| 挿入位置 | H2が2つ以上なら、章数の中央にあたるH2の直前（8章・9章なら第5章の直前） |
| 表示ルール | ワイド画面は「本文内 + サイドバー」、狭い画面は「本文内 + 読了後」。サイドバーと読了後の大型CTAは同時に表示しない |
| クリック計測 | 現在は計測基盤がないため送信しない。将来の `blog_cta_click` 用に `data-cta-id` / `data-cta-category` を付与済み |

- 本文の見出し数から位置を決めるため、CTAは見出し階層や目次項目には含まれません。
- 記事内容に直接必要な問い合わせ先を通常の文章リンクとして案内することはできますが、自動CTAの前後で同じ案内を重ねないでください。

### 見出しの使い分け

見出しは**装飾ではなく文書の構造**です。見た目を変えたいという理由でレベルを飛ばさないでください（目次と読み上げの構造が壊れ、SEO にも不利になります）。

| レベル | 書き方 | 役割 | 見た目 | 目次 |
| --- | --- | --- | --- | --- |
| H1 | frontmatter の `title` | 記事タイトル。**本文で `# ` は使わない** | ページ最上部の大見出し | — |
| H2 | `## ` | 章。話題が変わる区切り | 薄いブルーの塗り＋太い左バー | 載る |
| H3 | `### ` | 節。章の中の区切り | 左バー＋下線 | 載る |
| H4 | `#### ` | 項。節をさらに分ける | 太字（装飾なし） | 載らない |

- 本文の最上位は必ず **H2** から始めます。
- H4 まで必要になったら、章立て自体が細かすぎないか見直してください。
- 見出しにマーカーやリンクは使いません。

### リスト・引用・表の使い分け

| 要素 | 使いどころ | 書き方 |
| --- | --- | --- |
| 箇条書き（ul） | 順序に意味がない並列の要素 | `- ` |
| 番号付き（ol） | 手順・順位・件数に意味があるもの | `1. ` |
| 引用（要点カード） | 本文から浮かせたい要点・注記・補足・出典 | `> ` |
| 表 | 2 軸以上の比較 | GFM の表記法 |

- **箇条書きの入れ子は 1 段まで。** 2 段以上必要なら見出しで割るか表にします。
- **引用は「要点カード」**として描画されます。1 行目を `**要点**` `**注意**` のように太字ラベルにすると見出し行になります。記事冒頭に 3 点程度の要点を置くと読了率が上がります。
- **表は列が多くても潰れません。** 画面に収まらない場合は表だけが横スクロールします。ただしスマホでの読みやすさを考えると 3 列程度までが実用的です。
- 区切り線 `---` は多用しないでください。章の区切りは H2 で表します。

### 重要箇所のマーカー

結論やキーワードなど、流し読みでも拾ってほしい箇所は `==` で囲むとマーカー付きで表示されます。

```markdown
成果を出す企業は、==データの一元化==から始めています。
```

使い方のルール:

- **使用目安は 1 段落に 1 箇所、1 セクション（`##`）に 2〜3 箇所まで。** 増えるほど 1 箇所あたりの効果は薄れます。
- マーカーを外しても文章の意味が通るように書きます（音声読み上げでは通常の本文として読まれます）。
- 対象は**結論・数値・キーワード**などの短いフレーズ。文全体を塗るのは避けます。
- `==` の内側の端に空白を入れると変換されません（`== こう ==` は不可、`==こう==` と書く）。
- `==**太字入り**==` のように内側で他の記法も使えます。コード内の `==` は変換されません。

### 画像の入れ方

画像は記事フォルダの `assets/` に置き、**相対パス**で参照します。

```markdown
![図1の説明](assets/figure-1.png)
```

カバー画像を使う場合は frontmatter に `cover: assets/cover.png` を指定します。指定しなければカテゴリ別の抽象アートが自動で表示されます。

## 4. PR を作る

```bash
git checkout -b blog/your-article-slug
git add public/blog/your-article-slug
git commit -m "blog: <記事タイトル>"
git push -u origin blog/your-article-slug
```

GitHub で PR を作成してください。レビュー後マージされると公開されます。ローカルで確認するには `npm run dev` で `/blogs` を開きます。
