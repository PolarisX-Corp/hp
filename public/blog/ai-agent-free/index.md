...
<!-- 関連記事（公開後に結線）: AIエージェント／AI社員の定義そのものは pillar /blogs/ai-employee へ、ChatGPTベースのエージェントの定義・作り方は /blogs/ai-agent-chatgpt へ -->
...
<!-- 図指示: fig1-table-free-boundary（型ID: table／比較・位置づけ）。2列の境界表「無料で体験できる（左・青）」vs「有料/自作が要る（右・グレー〜赤アクセント）」。左=議事録要約・情報収集とレポート下書き・単発の調べもの・簡単なFAQ試作。右=止まらない安定運用・高性能モデルでの精度・チーム/権限管理・監査ログ・機密情報/法令対応。中央に対比軸ラベル「相性の見極め ↔ 恒常運用」。スタイルはpaper基調（直近ai-employeeのflat/signalとローテ）。1600×900。 -->
...
<!-- 図指示: fig2-matrix-usecase（型ID: matrix／比較・位置づけ）。2軸4象限。横軸=用途（左:情報収集・要約 → 右:自社独自ワークフロー）、縦軸=構築難易度（下:そのまま使う → 上:開発が必要）。左下=そのまま使う自律型/生成AI付属、右下=ノーコード構築（Difyなど代表例）、右上=開発者向け（OSS/コーディング支援）、左上=サブスク付属の高度機能。各象限に一言。paper基調・青3レンジ。1600×900。 -->
...
<!-- 図指示: fig3-hierarchy-agent-types（型ID: hierarchy／構造・関係）。3段の階層図。上段=そのまま使う自律型（設定不要・情報収集/要約）、中段=ノーコード構築型（自社仕様を画面操作で・Dify/Coze等を代表例）、下段=開発者向け/コーディング支援型（OSS・コード補助・技術力が要る）。右側に「必要スキル: 低→高」の帯。優劣ではなく段階であることを矢印で示す。paper基調。1600×900。 -->
...
<!-- 関連記事（公開後に結線）: 無料×ノーコードで"作る"手順は /blogs/ai-agent-diy へ、Dify無料の詳細は /blogs/dify-guide へ -->
...
<!-- 図指示: fig4-signal-how-far（型ID: signal／量・基準）。信号機（青/黄/赤）で「どこまで無料で足りるか」を判定。青=無料で足りる（PoC・単発業務・少人数・機密なし）、黄=条件つき（回数がやや不足・精度に不満・連携を増やしたい）、赤=有料/自作へ（回数上限に頻繁到達・高性能/高速が要る・他部署展開・機密情報/法令要件）。赤のみ #E0405E、他は青系。判定の一言を各灯に添える。paper基調。1600×900。 -->
...
<!-- 図指示: fig5-checklist-pitfalls（型ID: checklist／文書・人）。無料版の落とし穴チェックリスト（□つきカード6項目）。(1)回数・メッセージ上限で業務が止まる (2)無料プランは学習オフにできない場合がある (3)機密情報の入力ルールが未整備＝シャドーAI (4)無料が予告なく有料化/終了しうる (5)一世代前のモデルで精度が頭打ち (6)チーム/権限管理・監査ログが無い。sketch基調（手書き風・チェックリストに合う）で強調スタイルを1つだけ変える。1600×900。 -->
...
<!-- 関連記事（公開後に結線）: 無料版セキュリティの深掘りは /blogs/ai-agent-security へ -->
...
<!-- 関連記事（公開後に結線）: 有料も含めた全体比較は /blogs/ai-agent-comparison へ、無料×ノーコードで作る手順は /blogs/ai-agent-diy へ、AIエージェント/AI社員の定義は /blogs/ai-employee へ、導入相談は /blogs/ai-consulting・/blogs/sme-ai-efficiency へ、社内ナレッジは /blogs/knowledge-management-tools へ -->
...
<!-- 図指示: fig6-decision-selection-flow（型ID: decision／構造・関係）。決定木。START「無料で試す」→Q1 用途は1つに決まっているか(No→用途を1つ選ぶへ戻る)→Q2 無料で相性が見えたか(No→立ち止まる/別用途)→Q3 回数上限・精度不足・他部署展開・機密/法令のいずれかが立つか(No→無料を継続)→Yes→Q4 社内固有文脈・独自ワークフロー・権限/ログ要件が加わるか(No→有料へ / Yes→自作・導入へ)。paper基調・Yes/No分岐を青、終点「自作・導入へ」を締め色。1600×900。 -->
