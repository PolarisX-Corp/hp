...
<!-- 図指示 fig1（型ID: tree／構造・関係）: 起点ノード「AIで何をしたいか（自社のフェーズ）」から4分岐する見取り図。枝ラベル=「課題がまだ曖昧」「専用の仕組みをPoCから作る」「業務に根付かせ運用まで」「大規模・基幹システム連携」→ 到達ノード=4タイプ（各タイプに一言説明）。個別社名は載せない。paper基調（明朝見出し・罫線）・ライト。1600×900。 -->
...
<!-- 将来内部リンク（公開後に curator が結線）: /blogs/ai-employee（AI社員・自社運用に載る構築とは＝CV導線の主リンク）, /blogs/ai-agent-dev-company（AIエージェント構築の受託・費用に特化）, /blogs/ai-consulting（AIコンサルの役割の深掘り） -->
...
<!-- 図指示 fig2（型ID: checklist／文書・人）: 7軸のチェックカード。各カードに軸名＋確認する問いを1行（例: 自社データ=「社内文書・ナレッジを参照させる設計（RAG）が提案に入っているか」、運用伴走=「納品後に誰が使われ方を見て改善するか」）。チェックボックス付き・2列×4段（1枠は補足）。paper基調・ライト。1600×900。 -->
...
<!-- 将来内部リンク: /blogs/knowledge-management-tools（選定軸4の深掘り＝自社データ・RAGの整備）, /blogs/ai-adoption-support（導入支援サービスという選択肢） -->
...
<!-- 図指示 fig3（型ID: ladder／流れ・時間）: フェーズ別費用の階段。PoC（100万〜500万円・1〜3か月）→小規模（500万〜1,000万円）→本番（1,000万〜3,000万円以上）→大規模（3,000万〜1億円以上）。本番の手前に「ここで投資判断（PoCの結果で撤退基準を確認）」フラグ。脚注に「出典: GeNEE・LION AI各社解説の相場帯・変動あり」。paper基調・ライト。1600×900。 -->
...
<!-- 図指示 fig4（型ID: matrix／比較・位置づけ）: 4象限。横軸=フェーズ（左:探索・検証／右:本番・定着）、縦軸=アプローチ（下:既製を運用に載せる／上:ゼロから作る）。左上=受託開発型でPoC、右上=受託開発・大手SIer型（本番開発）、左下=既製SaaS・ツールの試験導入、右下=生成AI・SaaS型（運用伴走・AI社員）。各象限に一言の目安。paper基調・ライト。1600×900。 -->
...
<!-- 将来内部リンク: /blogs/chatgpt-enterprise(既製ツールの導入で足りる場合）, /blogs/ai-agent-comparison（AIエージェント自体のタイプ比較）, /blogs/sme-ai-efficiency（中小企業の業務効率化の受け皿） -->
...
<!-- 図指示 fig5（型ID: contrast／比較・位置づけ・本記事のsignature図）: 対比2カラム。左=「作って終わりの受託」（納品がゴール／業務が変わっても仕組みは固定／使われず陳腐化・追加改修は都度見積もり）、右=「自社運用に載る構築」（運用開始がスタート／自社ナレッジを取り込み精度が育つ／使われ方を観測して改善が回る）。中央に対比軸「納品後、誰が改善を回すか」。強調枠=R3（codex/gpt-image-2）の文字なし下絵をカードヘッダーに敷く（左: 埃をかぶった箱型マシン、右: ロボットと人が並走して knowledge が育つ様子）。和文はHTML合成。ライト・paper基調。1600×900。 -->
...
<!-- 図指示 fig6（型ID: decision／構造・関係）: 選定フローの決定木。開始「課題は明確か？」→No=戦略コンサル型／Yes→「既製のSaaS・AIプロダクトで足りるか？」→Yes=既製ツール導入（定着まで求めるなら生成AI・SaaS型）／No→「ゼロから作る必要があるか？」→Yes→「大規模・基幹連携か？」→Yes=大手SIer型・No=受託開発型（PoCから）／最下段に共通条件「運用・改善を自社で回せないなら、どの経路でも運用伴走を契約に含める」。Yes/Noエッジラベル。paper基調・ライト。1600×900。 -->

<!-- 将来内部リンク: /blogs/ai-employee（自社運用に載る構築＝AI社員とは）, /blogs/ai-consulting（AIコンサルの選び方）, /blogs/ai-adoption-support（導入支援サービス） -->
