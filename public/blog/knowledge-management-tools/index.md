...
<!-- 図指示 fig1（型ID: tree／構造・関係／style: paper）: 4タイプの分類ツリー。根ノード「ナレッジマネジメントツール（組織の知識を貯めて引き出せるようにするツールの総称）」→4分岐: FAQ型（Q&Aペア／問い合わせ削減）・社内Wiki型（自由な文書／手順・議事録の共有）・ドキュメント管理型（版管理・承認／正式文書）・AI・RAG型（AIが読んで答える／横断検索の発展形）。個別製品名は載せない。 -->

<!-- 将来内部リンク: /blogs/ai-helpdesk /blogs/faq-chatbot（FAQ型の代表ユースケース＝問い合わせ自動化）, /blogs/gemini-manual-creation /blogs/ai-manual-creation（マニュアル作成の手順） -->
...
<!-- 図指示 fig2（型ID: radar／量・基準／style: paper）: 6軸のレーダー（六角形の枠のみ・製品の評価線は描かない＝「軸そのもの」の提示）。各頂点に軸名＋確認の問い1行（例: 検索性=「探して3回見つからないと人は開かなくなる」）。 -->
...
<!-- 図指示 fig3（型ID: beforeafter／比較・位置づけ／style: illust=R3下絵＋HTML和文合成・記事内の強調1枚）: 左パネル「これまで＝人が探して読む」（人が資料の山・検索窓と向き合うイラスト）→右パネル「これから＝AIが読んで答える(RAG)」（AIがナレッジベースを参照し、出典つきで回答するイラスト）。中央に矢印。ナレッジベースが「AIの一次ソース」に変わることを1メッセージで。 -->
...
<!-- 将来内部リンク: /blogs/chatgpt-rag（ChatGPT×社内ナレッジの実装）, /blogs/rag-service-comparison（RAG特化サービス比較）, /blogs/dify-knowledge-base（Difyでの構築）, /blogs/ai-employee（AI社員＝ナレッジベースを読む脳）, /blogs/ai-agent-comparison（AIエージェント比較＝エージェントの精度はナレッジ源に依存）, /blogs/chatgpt-enterprise（法人ChatGPT） -->
...
<!-- 図指示 fig4（型ID: matrix／比較・位置づけ／style: paper）: 4象限マップ。横軸=扱うナレッジ（定型のQ&A ⇄ 自由な文書）、縦軸=活用の主体（人が検索して読む ⇄ AIが読んで答える）。4タイプを配置し、AI・RAG型の象限を薄くハイライト（これからの主戦場）。上のmarkdown表（軸×タイプの◎△）とは情報を変え、「位置づけの地図」に徹する。 -->
...
<!-- 図指示 fig5（型ID: persona／文書・人／style: paper）: ケース別カード4枚。各カードに「会社の状況」（例:「CSに同じ質問が集中」「エースのノウハウが個人持ち」「契約書の版が散らばる」「AIに社内のことを聞きたい」）→「推奨タイプ」を対で。人物/部署のピクトグラム（R2 SVG）を添える。 -->
...
<!-- 将来内部リンク: /blogs/ai-development-company（開発会社の選び方）, /blogs/ai-consulting（導入相談）, /blogs/internal-rag /blogs/internal-ai-diy（自作・構築費用） -->
...
<!-- 図指示 fig6（型ID: loop／流れ・時間／style: paper）: ナレッジ運用の円環ループ「貯める→整える→引く（人・AI）→使って直す→（また貯まる）」。円環上の3か所に断点マーク（赤・限定使用）: ①更新の担い手がいない（整える→の断点）②AIが読めない形式（引くの断点）③検索性が低い→エースに聞くに戻る（引く→使うの断点）。循環が回る=定着、断たれる=形骸化。 -->

<!-- 将来内部リンク: /blogs/multimodal-rag(画像・図表を含む文書のRAG) -->
...
<!-- 図指示 fig7（型ID: decision／構造・関係／style: paper）: 選定フローの決定木。起点「いちばん解決したい課題は？」→4分岐で各タイプへ→合流して「AIから読める形にする？」（Yes→4条件を満たす製品・設計）→「情シス・運用体制は？」（なし→運用しやすさ優先／外部と組む）→終点「1部門・1用途でスモールスタート」。ai-agent-comparison の決定木（横レール型）とはレイアウトを変え、縦の分岐ツリーで描く。 -->
