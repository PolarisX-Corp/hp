...
<!-- 図指示 fig1（型ID: hierarchy／構造・関係）: 4タイプの見取り図。上位ラベル「AIエージェント（自律的にタスクを実行）」から4分岐: 汎用作業型（調査・下書き・要約）／業務特化型（経理・営業など特定業務）／ワークフロー連携型（複数の定型処理を連結・API/MCP）／カスタム構築型（社内文脈を深く使い複数部門で継続運用・司令塔）。個別製品名は載せない。--accent 基調、1図1メッセージ。 -->
...
<!-- 将来内部リンク: /blogs/multi-ai-agent（複数エージェント連携・司令塔）, /blogs/ai-employee（AI社員とは・違いの定義の深掘り） -->
...
<!-- 図指示 fig2（型ID: checklist／文書・人）: 6軸のチェックカード。各カードに軸名＋確認する問いを1行（例: 自社データ=「社内ナレッジ/RAGを参照させられるか」、セキュリティ=「国内処理・学習不使用・ログ保管を確認できるか」）。チェックボックス付き。--accent 基調。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-security（セキュリティ深掘り）, /blogs/knowledge-management-tools（社内データ・RAGの持ち込み） -->
...
<!-- 図指示 fig3（型ID: heatmap／比較・位置づけ）: 行=4タイプ、列=用途の広さ/自社文脈の持ち込み/外部連携/初期の重さ/従量の効き方。セルは◎○△を濃淡で。個別製品名は入れない。読み方の矢印（重視する軸の列を縦に見る）を添える。 -->
...
<!-- 図指示 fig4（型ID: matrix／比較・位置づけ）: 4象限。横軸=業務の定型度（左:非定型／右:定型）、縦軸=社内文脈への依存（下:低／上:高）。右上=カスタム構築型（司令塔）、右下=ワークフロー連携型・業務特化型、左下=汎用作業型、左上=非定型×文脈依存高は「まず業務の型づくり」。--accent と --muted で塗り分け。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-free（無料で試す）, /blogs/ai-agent-chatgpt（ChatGPTでの実装）, /blogs/ai-agent-dev-company（開発会社・カスタム構築を頼む） -->
...
<!-- 図指示 fig5（型ID: signal／量・基準）: 信号機3段階。青=「社内ナレッジ整備＋窓口一元化＋確認体制あり=効く」、黄=「役割分担が曖昧／従量コスト未確認=条件つき」、赤=「RAG未整備・窓口分裂=精度が出ない」。否定側のみ赤 #E0405E を限定使用。 -->
...
<!-- 図指示 fig6（型ID: decision／構造・関係）: 選定フローの決定木。開始「用途は？」→分岐（横断作業/特定業務/複数処理の連結/複数部門で継続）→「自社データを持ち込む？」→「情シスは？」→「PoCで試す」→到達ノード=4タイプ。Yes/Noのエッジラベル。--accent 基調。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-diy（作り方）, /blogs/ai-agent-free（無料で試す）, /blogs/ai-agent-chatgpt（ChatGPTでの実装）, /blogs/ai-consulting, /blogs/sme-ai-efficiency（導入相談） -->
