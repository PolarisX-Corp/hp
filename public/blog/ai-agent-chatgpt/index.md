...
<!-- 図: メッセージ=チャットとエージェントの位置づけの対比。型=contrast（比較・位置づけ）。要素: 左カラム「通常のChatGPT（チャット）＝回答するAI／一問一答／手を動かすのは人」、右カラム「エージェントモード＝実行するAI／目的を渡すと自律実行／ブラウザ操作・コード実行・成果物生成」。中央に対比軸ラベル「"回答"か"実行"か」。 -->
...
<!-- 図: メッセージ=できることの全体像を4系統で整理。型=hierarchy（構造・関係）。上位ノード「ChatGPTエージェントでできること」→4系統「調べる（Web横断調査・情報収集）」「作る（スライド・スプレッドシート・レポート）」「つなぐ（Google Drive等の外部データ・アプリ連携）」「実行する（仮想PCでのコード実行・データ分析）」。各系統に代表例を1〜2個。 -->
...
<!-- 図: メッセージ=4つの概念の境界と、ChatGPTエージェントの重なり。型=venn（構造・関係）。円: 「生成AI＝回答する」「AIエージェント＝目的から実行する」「RPA＝固定手順を反復」「チャットボット＝定型応答」。ChatGPTエージェントのマーカーを生成AIとAIエージェントの重なりに置く。RPA・チャットボットは範囲が限定的であることを注記。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-comparison（ツール横断の比較・選定）, /blogs/ai-employee（AIエージェント/AI社員の一般定義・pillar） -->
...
<!-- 図: メッセージ=3ルートを手軽さ↔作り込み度で並べる。型=steps（流れ・時間）。左から右へ「ルート1 エージェントモードを使う（設定不要・今日から）」→「ルート2 GPTsで自社専用を作る（ノーコード・作成はPlus以上）」→「ルート3 APIで業務に組み込む（要開発・恒常的な自動化）」。各ルートの下に"向く目的"を一言（まず試す／社内で共有する定型業務／業務システムに組み込む）。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-diy（GPTsでの作り方・step-by-step全手順） -->
...
<!-- 将来内部リンク: /blogs/multi-ai-agent（司令塔/複数エージェント設計）, /blogs/chatgpt-rag（ChatGPT×社内ナレッジ・RAG） -->
...
<!-- 図: メッセージ=プランが上がるほど使える機能が増える段階。型=ladder（流れ・時間｜プラン階段）。段=無料（公開GPTs利用のみ・エージェントモード/GPTs作成は×）→Plus 月額20ドル前後（エージェントモード＆GPTs作成が解禁＝分かれ目）→Pro 上位（実行回数が多い）→Business・Enterprise 法人（チーム管理・全社運用）。金額は"目安"で変動する旨のキャプション＋直後のmarkdown表と補完。※editor申し送りの table→ladder 差替を適用（ai-employee との table 型重複を回避）。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-free（無料で始める選定）, /blogs/chatgpt-business, /blogs/chatgpt-enterprise（法人ChatGPT） -->
...
<!-- 図: メッセージ=タスク別の"そのまま任せられる度"。型=gauge（量・基準）。横帯のゲージ。高（青）側に「定型の調査・情報収集」「資料の下書き・一次集計」、中に「外部連携を伴う作業（人の監督下）」、低（赤）側に「判断・法務・契約・個人情報が絡む業務＝人の確認前提」。基準線として"人の最終確認が要るライン"を引く。 -->
...
<!-- 将来内部リンク: /blogs/ai-agent-security（セキュリティ深掘り） -->
...
<!-- 将来内部リンク: /blogs/multi-ai-agent（司令塔設計）, /blogs/chatgpt-rag（社内ナレッジ接続） -->
...
アーキタイプ: A. 定義解説型。配色/フォント/brandbar は figure-patterns.md 共通デザインシステムに従う。ロゴは ../../../brand/PolarisX_wordmark.svg。サイズは本文図=1600×900@2x、cover=1200×630@2x。
基調スタイル: ai-employee が paper 基調＝直近ローテのため、本記事は flat 基調＋1枚だけ illust 強調（fig1 contrast のヒーロー性を上げる）を推奨。全図同一スタイルのベタ塗りを避ける。

- cover.png … シンプル明色。タイトル全文「ChatGPTのAIエージェントとは？違い・使い方・料金を解説」を手動改行で2〜3行。「AIエージェント」を .em（--accent #366EFF）。装飾・カテゴリチップ・タグラインは付けない。正本テンプレは ai-ready-data-platform/assets/cover.html を複製して文言差し替え。

- fig1-contrast-chat-vs-agent.png（型ID: contrast／比較・位置づけ）… H2「ChatGPTのAIエージェントとは」。左右2カラム対比。左「通常のChatGPT（チャット）＝回答するAI／一問一答／手を動かすのは人」、右「エージェントモード＝実行するAI／目的を渡すと自律実行／ブラウザ操作・コード実行・成果物生成」。中央に対比軸ラベル「"回答"か"実行"か」。1図1メッセージ。

- fig2-hierarchy-capabilities.png（型ID: hierarchy／構造・関係）… H2「できること」。上位ノード「ChatGPTエージェントでできること」→4系統ノード【調べる（Web横断調査・情報収集）】【作る（スライド・スプレッドシート・レポート）】【つなぐ（Google Drive等の外部データ・アプリ連携）】【実行する（仮想PCでのコード実行・データ分析）】。各系統に代表例1〜2個。

- fig3-venn-boundary.png（型ID: venn／構造・関係）… H2「生成AI・RPA・チャットボットとの違い」。円: 生成AI（回答する）／AIエージェント（目的から実行する）／RPA（固定手順を反復）／チャットボット（定型応答）。ChatGPTエージェントのマーカーを「生成AI×AIエージェント」の重なりに配置。RPA・チャットボットは範囲が限定的である注記。

- fig4-steps-three-routes.png（型ID: steps／流れ・時間）… H2「3つのルート」。左→右で作り込み度が上がる3ステップ。【ルート1 エージェントモードを使う｜設定不要・今日から｜まず試す】→【ルート2 GPTsで自社専用を作る｜ノーコード・作成はPlus以上｜社内で共有する定型業務】→【ルート3 APIで業務に組み込む｜要開発・恒常的な自動化｜業務システムに組み込む】。

- fig5-table-plan-matrix.png（型ID: table／比較・位置づけ）… H2「料金の目安と無料でできる範囲」。行=プラン（無料／Plus 月額20ドル前後／Pro 上位／Business・Enterprise 法人）、列=エージェントモード／GPTsの作成／公開GPTsの利用。無料=前2列×・最終列○（回数制限）、Plus以上=全て○（Pro は実行回数が多い注記）。金額は"目安"で変動する旨のキャプション。※ai-employee が table を1枚使用済み＝原則避ける警告レベル。重複が気になる場合は ladder（プラン階段: 無料→Plus→Pro→法人）へ差し替え可。差し替え時はファイル名・型IDも fig5-ladder-plan-steps.png に更新し本文参照も合わせること。

- fig6-gauge-delegation.png（型ID: gauge／量・基準）… H2「限界・注意点」。横帯ゲージ。高（青）側=定型の調査・情報収集／資料の下書き・一次集計、中=外部連携を伴う作業（人の監督下）、低（赤）側=判断・法務・契約・個人情報が絡む業務（人の確認前提）。基準線「人の最終確認が要るライン」。

型カテゴリ充足: contrast(比較・位置づけ)／hierarchy(構造・関係)／venn(構造・関係)／steps(流れ・時間)／table(比較・位置づけ)／gauge(量・基準)＝4カテゴリ・型重複なし（seo-lint準拠）。直近 ai-employee が使った型（anatomy/matrix/checklist/bignumber/table/signal）と極力ずらし、重複は table のみ（差し替えオプション明記）。
-->
